...
import dev.soul.more.MoreState
import dev.soul.more.MoreViewModel
import dev.soul.more.components.StadiumItem
...
fun MoreRoot(
    ...
    Scaffold(
        ...
        topBar = {
            BaseToolbar(
                name = if (state.isPopular == true) "Popular" else "Personalized",
                onBack = {
                    onBack()
                }
            )
...
internal fun Content(
    ...
    LazyColumn(
        ...
        itemsIndexed(
            if (state.isPopular == true) state.popularList else state.personalizedList,
            key = { index, _ -> index }
        ) { index, item ->
            StadiumItem(
